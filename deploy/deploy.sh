...
if ! git remote get-url o2switch >/dev/null 2>&1; then
  echo "==> Adding git remote 'o2switch'"
  git remote add o2switch "ssh://${SSH_USER}@${SSH_HOST}:${SSH_PORT}/~/${REMOTE_GIT_DIR}"
fi

BRANCH="$(git rev-parse --abbrev-ref HEAD)"
echo "==> Pushing '$BRANCH' to o2switch (this triggers the remote build — can take a few minutes)"
git push o2switch "${BRANCH}:main"
